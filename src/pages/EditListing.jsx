...
export default function EditListing() {
  ...
  // In useEffect where we fetch the listing data
  useEffect(() => {
    setLoading(true);
    async function fetchListing() {
      ...
      if (docSnap.exists()) {
        ...
        setFormData({ ...docSnap.data(), images: {} }); // Initialize images as empty object
        setLoading(false);
      } else {
        navigate("/");
  ...
  // In onSubmit function, add a check for images
  async function onSubmit(e) {
    ...
    // Only check images if new ones were selected
          ...
          (snapshot) => {
            ...
            switch (snapshot.state) {
              ...
              case "running":
                ...
                break;
              default:
                console.log("Upload is in state:", snapshot.state);
                break;
            }
          },
          (error) => {
    ...
    // Only process images if new ones were selected
    let imgUrls = listing.imgUrls; // Keep existing images by default
    if (images && images.length > 0) {
      imgUrls = await Promise.all(
        ...
        return listing.imgUrls; // Keep existing images if upload fails
      });
    }
